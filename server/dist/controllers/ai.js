...
exports.generateFromPdf = void 0;
...
const generateFromPdf = async (req, res) => {
    ...
    try {
        ...
        const titlePrompt = `Based on the following text content, generate a concise and descriptive title for a flashcard set. The title should be 3-8 words and capture the main topic. Only return the title, nothing else. Text: ${text.substring(0, 1000)}...`;
        ...
        await Folder_1.default.findByIdAndUpdate(folder._id, { name: generatedTitle });
        await Card_1.default.insertMany(flashcards.map((card) => ({
            front: card.question, // Map question to front
            back: card.answer, // Map answer to back
            folder: folder._id,
            user: userId, // Add required user field
        })));
        await Folder_1.default.findByIdAndUpdate(folder._id, { status: 'completed' });
    }
    catch (error) {
        console.error('Error generating flashcards:', error);
        await Folder_1.default.findByIdAndUpdate(folder._id, { status: 'failed' });
    }
};
exports.generateFromPdf = generateFromPdf;
